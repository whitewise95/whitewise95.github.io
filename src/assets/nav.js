(() => {
  ...
  const careerDetails = [...document.querySelectorAll("[data-career-accordion]")];
  careerDetails.forEach((details) => {
    details.addEventListener("toggle", () => {
      if (!details.open) return;
      careerDetails.forEach((other) => {
  ...
  if (slides.length) {
    ...
    showSlide(index, { historyMode: "replace" });
    return;
  }
