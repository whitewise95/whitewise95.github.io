...
```'
    diff_text="Java 7까지는 익명 클래스와 반복문 중심 코드가 많았지만, Java 8부터는 함수형 추상화가 표준으로 자리잡았습니다. 특히 API 설계 시 Stream, Optional, java.time을 기본 전제로 보는 관점 변화가 가장 큽니다."
    ;;
  9)
    release_year="2017"
    summary_table="| 항목 | 내용 |\n|---|---|\n| 모듈 시스템 | Jigsaw(module-info.java)로 런타임/컴파일 의존성 명시 |\n| REPL 도구 | JShell 추가로 스크립트형 실험과 학습 생산성 향상 |\n| 컬렉션 팩토리 | List.of, Set.of, Map.of로 불변 컬렉션 생성 간소화 |\n| 스트림 개선 | takeWhile, dropWhile, iterate 오버로드 추가 |\n| Optional 개선 | ifPresentOrElse, or, stream 추가 |"
...
Java ${next_version}의 핵심은 "새 기능 추가" 그 자체보다, **코드 품질과 운영 안정성을 어떻게 함께 개선할지**에 있습니다.
$next_teaser
EOF_MD
